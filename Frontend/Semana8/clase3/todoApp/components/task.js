class Task extends HTMLElement {
  ...
  connectedCallback() {
    ...
    let deleteButton = this.querySelector(".deleteButton");
    if (deleteButton) {
      deleteButton.addEventListener("click", function () {
        host.setAttribute("status", "deleted");
      });
    }
  }
  render() {
    ...
    const buttonCheck = () => {
      if (this.status == "created") {
        return `<a role="check" class="btn btn-sm btn-outline-primary py-0 small opacity-50 checkButton">&check;</a>`;
      } else {
        return `<a role="check" class="btn btn-sm btn-success py-0 small opacity-50 disabled">&check;</a>`;
      }
    };
    const showButtons = () => {
      if (this.status != "deleted") {
        return `<a class="btn btn-sm btn-outline-secondary py-0 small opacity-50">&#9998;</a>
